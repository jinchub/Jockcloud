...
-  **文件预览**：文本格式在线编辑，支持图片、文档、视频等格式预览，常见文档：Word、PDF、Excel、PPT
-  **文件同步**：本地文件夹与云端双向同步、增量同步、冲突处理
-  **私密空间**：私密文件、私密文件夹、无操作自动退出
-  **配额管理**：用户组方案存储空间配额、上传数量限制
-  **回收站**： 软删除机制、30 天自动清理、一键恢复
-  **空间管理**：储存盘选择，挂载 NFS 远程目录作为存储盘
...
- `id` - 用户组 ID
- `name` - 用户组名称
- `quota_bytes` - 用户组空间配额（-1 为无限制）
- `max_upload_size_mb` - 用户组上传大小限制
- `max_upload_file_count` - 用户组上传数量限制
