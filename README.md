...
- 🗑️ **回收站**：软删除机制、30 天自动清理、一键恢复
- 📱 **响应式设计**：支持 PC 端和移动端自适应
- 🌙 **深色模式**：支持深色/浅色/自动主题切换
- 📈 **系统监控**：CPU、内存、磁盘、网络实时监控
- ☁️ **云存储挂载**：支持阿里云 OSS、腾讯云 COS、七牛云作为存储后端
- 🛡️ **安全机制**：请求速率限制、短信验证码、登录会话管理
...
logInfo("操作成功", { userId: 1, action: "upload" });
logError("操作失败", { error: error.message });
```
...
// 插入
const [result] = await pool.query("INSERT INTO users (username) VALUES (?)", ["test"]);
const insertId = result.insertId;

// 更新
await pool.query("UPDATE users SET quota = ? WHERE id = ?", [quota, userId]);
...
```bash
# 备份整个数据库
mysqldump -u wall -p cloud > backup_$(date +%Y%m%d_%H%M%S).sql

# 压缩备份
mysqldump -u wall -p cloud | gzip > backup_$(date +%Y%m%d_%H%M%S).sql.gz
```
...
```bash
# 每天凌晨 2 点备份
0 2 * * * mysqldump -u wall -p'your_password' cloud > /backup/cloud_$(date +\%Y\%m\%d).sql

# 保留最近 7 天的备份
0 2 * * * find /backup -name "cloud_*.sql" -mtime +7 -delete
```
...
```bash
# 从 SQL 文件恢复
mysql -u wall -p cloud < backup_20260421.sql

# 从压缩文件恢复
gunzip < backup_20260421.sql.gz | mysql -u wall -p cloud
```
...
```bash
# 解压恢复
tar -xzf uploads_backup_20260421.tar.gz -C /path/to/jockcloud/
...
BACKUP_DIR="/backup/jockcloud"
DATE=$(date +%Y%m%d_%H%M%S)
DB_USER="wall"
DB_PASS="wall2025"
DB_NAME="cloud"
APP_DIR="/path/to/jockcloud"
...
1. **备份当前版本和数据**：
   ```bash
   # 备份数据库
   mysqldump -u wall -p cloud > backup_before_upgrade.sql
...
```sql
-- 查看用户配额
SELECT id, username, quota_bytes FROM users WHERE id = 1;

-- 修改配额（-1 表示无限制）
UPDATE users SET quota_bytes = -1 WHERE id = 1;
```
...
A: 在系统设置 → 登录配置中，关闭"短信验证码登录"开关即可。

### Q: 云存储挂载后无法访问？

A: 检查以下几点：
1. 确认 AccessKey 和 SecretKey 正确
2. 检查 Bucket 权限设置（需要读写权限）
3. 确认 Region 配置正确
4. 检查网络连接和防火墙设置
5. 查看错误日志获取详细错误信息
...
A: 尝试以下优化：
1. 使用 PM2 管理进程：`pm2 restart jockcloud --max-memory-restart 512M`
2. 调整 Node.js 内存限制：`export NODE_OPTIONS="--max-old-space-size=512"`
3. 检查是否有大量并发上传/下载任务
4. 定期清理临时文件和过期会话
...
1. **查看文档**：本文档已涵盖大部分使用场景
2. **查看日志**：`logs/error.log` 中通常有详细错误信息
3. **检查配置**：确认 `.env` 和系统设置配置正确
4. **社区支持**：如有开源社区可提问
